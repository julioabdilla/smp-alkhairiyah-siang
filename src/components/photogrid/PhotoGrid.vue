<template>
  <div v-if="large" class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 gap-4">
    <div v-for="(photo, index) in photos" :key="index" class="flex bg-gray-300 items-center">
      <img :src="photo.url" alt="Photo" class="object-contain w-full h-full mx-auto">
    </div>
    <a v-if="detailPage" :href="`#${detailPage}`" class="bg-gray-200">
    </a>
  </div>
  <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
